...
class ComplexQNetwork(BaseQNetwork):
    def __init__(self, model):
        ...
        self.linear_with_tau = torch.nn.Linear(3 * embed_dim, embed_dim)
    ...
    def compute_q_values(self, node_embeddings, raw_states, batch):
        ...
        taus = [
            state["tau"] if state["tau"] is not None else -1 for state in raw_states
        ]  # Use -1 as placeholder for None
        mask_state_has_tau = torch.tensor(
            [t != -1 for t in taus], dtype=torch.bool, device=self.device
        )  # compute mask for states with tau
        mask_state_has_tau_per_node = mask_state_has_tau[batch]  # broadcast to [N]
        taus = torch.tensor(
            [t if t != -1 else 0 for t in taus], dtype=torch.long, device=self.device
        )  # replace -1 with 0 so every state has a tau

        node_counts = torch.bincount(batch)  # get index of each graphs first node
        node_starts = torch.cat(
            [node_embeddings.new_zeros(1, dtype=torch.long), node_counts.cumsum(0)[:-1]]
        )  # shape: [batch_size]
        ...
        out = torch.where(
            mask_state_has_tau_per_node.unsqueeze(-1), out_tau, out_no_tau
        )  # Select output based on whether tau is present
