...
class DepolarizeSimple:
    def __init__(self, n, k=2, max_edges = None, generate_states=True):
        ...
        if generate_states:
            self.states, self.hash_to_index, self.starting_states = self._generate_states()
            self.state_idxes = list(range(len(self.states)))
        self.actions = list(range(self.n))
        current_state = None
    
    def _generate_states(self):
        """Generates all unique (G, sigma, tau, l) up to isomorphism and returns dictionary mapping state_hash to quadruple."""
        all_edges = list(itertools.combinations(range(self.n), 2))
        all_sigmas = [sigma for sigma in itertools.product([-1, 1], repeat=self.n) if len(set(sigma)) > 1]
        ...
        for r in tqdm(range(self.max_edges + 1), desc="Generating states"):
            for edge_subset in itertools.combinations(all_edges, r):
                ...
                for sigma in all_sigmas:
                    for tau in all_taus:
                        state_hash, G_perm, sigma_perm, tau_perm = self.state_hash(G, sigma, tau, return_permuted_tuple=True)
                        ...
                        if state_hash not in seen:
                            ...
                            if tau is None:
                                ...
                                state_counter += 1
                                if max_edges_left_to_add == self.k:
                                    starting_states.append(hash_to_index[(state_hash, 0)])
    ...
    def reset(self):
        ...
        self.current_state = self.starting_states[np.random.randint(len(self.starting_states))]
    ...
    def step(self, action, state = None):
        """Returns the nest state, reward, and whether the state is terminal given an action (and state)."""
        if state is None:
            state = self.current_state
        G, sigma, tau, l = self.states[state]
        if l == self.k:
